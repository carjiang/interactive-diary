...
class AsyncConversationalGPTBaseAgent(ConversationalGPTBaseAgent, AsyncBaseAgent):
    ...
    def batch_interact(self, prompts, temperature=0, max_tokens=1024, system_prompts = None, histories: List[List] = None):
        ...
        #### BATCHED TRANSCRIPT LOGGING 
        for prompt, system_prompt, history, response in zip(
            prompts,
            system_prompts,
            histories if histories is not None else [None] * len(prompts),
            responses
        ):
        ...
        #### BATCHED TRANSCRIPT LOGGING END
    ...
    def save_transcript_json(self, filepath="transcript_convo.json"):
        data = {
